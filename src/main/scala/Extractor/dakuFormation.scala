...
//    val sc = new SparkContext(new SparkConf().setAppName("test").setMaster("local"))
    ...
    sc.textFile(savePath + "/CombineDayUnion_aoi/1-" + path0.length.toString + "/part-*").map {
      line =>
        val x = line.split("\t")
    ...
    sc.textFile(savePath + "/CombineDayUnion_dev/1-" + path0.length.toString + "/part-*").map {
      line =>
        val x = line.split("\t")
...
//        // appHash 映射到 Tag, 写出到Tag_geq1
//        val Hash2Top = MapUtil.get_Hash2top5wIndex("/user/fan.yao/app_top50000", sc)
//        sc.textFile(savePath + "/CombineDayUnion_app/1-60/part-*")
//          .map( readAppFeat.line2Struct(_) ).map {
//          x => x._1 + "\t" + x._2.map(Hash2Top).filter(_ != "").mkString(" ").split(" ").map(_.toInt).distinct.sorted.mkString(" ")
//        }.filter {
//          line => line.split("\t").length > 1
//        }.repartition(num_partition.toInt).saveAsTextFile(savePath + "/CombineDayUnion/Top5w")
